...
namespace Mechadroids
{
    public class PatchOperation_SettingActive : PatchOperation
    {
        ...
        public override bool ApplyWorker(XmlDocument xml)
        {
            ...
            ModContentPack modContentPack = MechadroidsMod.mod.Content;
            for (int i = 0; i < settings.Count(); i++)
            {
                if (!MechadroidsMod.settings.IsValidSetting(settings[i]))
                {
                ...
                if (MechadroidsMod.settings.GetEnabledSettings.Contains(settings[i]))
                {
                    flag = true;
                    break;
                }
